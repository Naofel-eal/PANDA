...
import { AgentEventType, BlockerType, ContentType, ProcessTiming, RequestedFrom, ResumeTrigger } from "../lib/nud-constants.js"
...
console.log("[nud-tool] NUD tool file loaded")
...
function config() {
  if (!_config) {
    _config = {
      orchestratorUrl: requiredEnv("NUD_ORCHESTRATOR_URL"),
      workflowId: requiredEnv("NUD_WORKFLOW_ID"),
      agentRunId: requiredEnv("NUD_AGENT_RUN_ID"),
      stateFile: requiredEnv("NUD_AGENT_STATE_FILE")
    }
    console.log(`[nud-tool] Config resolved for run ${_config.agentRunId} (workflow ${_config.workflowId})`)
  }
  return _config
}
...
async function sendEvent(type, payload, terminal = false) {
  ...
  console.log(`[nud-tool] Sending ${type} for run ${agentRunId}`)
  ...
  console.log(`[nud-tool] Delivered ${type} for run ${agentRunId}`)
  if (terminal) {
    console.log(`[nud-tool] Scheduling process exit after terminal event ${type} for run ${agentRunId}`)
    scheduleProcessExit()
  }
...
export const report_progress = tool({
  description: "Send a non-terminal progress update to the NUD orchestrator.",
  args: {
