...
import { AgentEventType, BlockerType, ContentType, ProcessTiming, RequestedFrom, ResumeTrigger } from "../lib/devflow-constants.js"
...
async function sendEvent(type, payload, terminal = false) {
  ...
  console.log(`[devflow-tool] Sending ${type} for run ${agentRunId}`)
  ...
  console.log(`[devflow-tool] Delivered ${type} for run ${agentRunId}`)
  if (terminal) {
    console.log(`[devflow-tool] Scheduling process exit after terminal event ${type} for run ${agentRunId}`)
    scheduleProcessExit()
  }
...
export const report_progress = tool({
  description: "Send a non-terminal progress update to the Devflow orchestrator.",
  args: {
