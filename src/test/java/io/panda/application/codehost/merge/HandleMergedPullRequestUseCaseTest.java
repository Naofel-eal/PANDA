package io.panda.application.codehost.merge;
...
class HandleMergedPullRequestUseCaseTest {
    ...
    @Test
    @DisplayName("Given a ticketing failure when a merged pull request is processed then PANDA swallows the error to keep polling alive")
    void givenTicketingFailure_whenMergedPullRequestIsProcessed_thenPANDASwallowsTheErrorToKeepPollingAlive() {
        HandleMergedPullRequestUseCase useCase = new HandleMergedPullRequestUseCase();
