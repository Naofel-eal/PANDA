...
class TicketingPortTest {
    ...
    @Test
    @DisplayName("Given a ticketing port with no specific lookup support when PANDA asks for ticket details then empty defaults are returned")
    void givenATicketingPortWithNoSpecificLookupSupport_whenPANDAAsksForTicketDetails_thenEmptyDefaultsAreReturned() {
    ...
    @Test
    @DisplayName("Given a minimal ticketing port when PANDA uses write operations then the contract stays business-focused")
    void givenAMinimalTicketingPort_whenPANDAUsesWriteOperations_thenTheContractStaysBusinessFocused() {
        TicketingPort ticketingPort = new TicketingPort() {
            @Override
            public void comment(CommentWorkItemCommand command) {
